...
extern std::string W2UTF8(const wchar_t* pszText, int nLength)
{
  ...
  //Now recall with the buffer to get the converted text
	std::string sUTF(nUTF8Length, '\0');
	char* pszUTF8Text = (char*)sUTF.data(); //include an extra byte because we may be null terminating the string ourselves
...
//	  pszUTF8Text[nCharsWritten] = '\0';
	...
	if (pszUTF8Text > 0 && pszUTF8Text[nCharsWritten] == 0)
		sUTF.resize(nCharsWritten);
	else
		sUTF.clear();
...
{
	int len= GetTextLength(true);
	std::string buf(len + 1, '\0');
	GetText(len + 1, &buf[0], true);

	int length= ::MultiByteToWideChar(CP_UTF8, 0, buf.c_str(), len, 0, 0);
	CString str;
	if (length > 0)
	{
		str.resize(length);
		::MultiByteToWideChar(CP_UTF8, 0, buf.c_str(), len, const_cast<TCHAR*>(str.data()), length);
	}

	return str;
}

std::string CScintillaCtrl::GetUTF8Text()
{
	int len= GetTextLength(true);
...
{
  //Convert the unicode text to UTF8
	std::string sUTF8(W2UTF8(text, -1));

  //Call the native scintilla version of the function with the UTF8 text
	SetText(sUTF8.c_str(), bDirect);
}
