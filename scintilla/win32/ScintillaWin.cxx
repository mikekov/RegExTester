...
void FillSolidRect(HDC hdc, int x, int y, int width, int height, COLORREF c)
{
	::SetBkColor(hdc, c);
	RECT rect;
	rect.left = x;
...
bool DrawWndElement(HDC hdc, RECT rect, const wchar_t* wnd_class, int part, int state, int exclude_interior)
{
	bool done= false;
...
sptr_t ScintillaWin::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	try {
		...
		case WM_GETDLGCODE:
			...
			return /*DLGC_HASSETSEL |*/ DLGC_WANTARROWS | DLGC_WANTCHARS | DLGC_WANTALLKEYS;
		...
		case WM_NCCALCSIZE:
			if (wParam)
			{
				...
				::InflateRect(p->rgrc, -BORDER, -BORDER);
			}
			return ::DefWindowProc(MainHWND(), iMessage, wParam, lParam);
		...
		case WM_NCPAINT:
			if (HDC hdc= ::GetWindowDC(MainHWND()))
			{
				// draw scrollbar
...
void ScintillaWin::UpdateSystemCaret() {
	...
	::RedrawWindow(MainHWND(), 0, 0, RDW_INVALIDATE | RDW_FRAME);	// update frame to reflect focus change
}
